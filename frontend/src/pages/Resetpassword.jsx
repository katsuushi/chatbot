...
function Resetpassword() {
    const [data, setData] = useState({ ps: "", cfps: "" });
    const [success, setSuccess] = useState(false);
    const [fieldEmpty, setFieldEmpty] = useState(false);
    const [psMatch, setPsMatch] = useState(true)
    ...
    async function handleCall() {
        ...
        } else if (data.ps != data.cfps) {
            setPsMatch(false)
        }
        const call = await fetch("http://localhost:8000/auth/reset-password", {
            method: "POST",
            headers: {
        ...
        if (call.ok) {
            ...
            if (res == { "detail": "RESET_PASSWORD_BAD_TOKEN" }) {
                setBadToken(true)
            }
        }
    }
            ...
            <div>
                ...
                {success ? (
                    <p className="text-xl! text-[#00FF00]! mb-0! mr-auto">
                        Password has been reset. Please try to log in again.
                    </p>
                ) : (
                    <></>
                )}
                ...
                {badToken ? (
                    <p className="text-xl! text-[#FF0000]! mb-0! mr-auto">
                        The reset token is expired. Please request a password reset again.{" "}
                    </p>
                ) : (
                    <></>
                )}
                ...
                <div className="text-white text-2xl my-8 flex">
                    <div className="flex flex-col items-center">
                        <label className="my-6">New Password:</label>
                        <label className="my-6">Confirm New Password:</label>
                    </div>
